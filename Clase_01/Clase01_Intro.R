...
imc<-peso/(talla^2)
vino.a.clase<-c(FALSE, TRUE, TRUE)
nombres_peliculas<-c("Inception", "Interstellar", "Across the Spiderverse")
genero_peliculas<-factor(c("Sci-fi", "Sci-fi", "Comic")) # Categorias

## Operaciones con vectores asignados
(a*b)/(b-a)

## Vectores de diferentes elementos (tipos de objetos)
class("TRUE") # Función que permite evaluar el tipo de objeto con el que trabajo
class(a)
...
var4<-c(T, F, T, F, F, T)
length(var1) #Length me permite saber el # de elementos de un vector
?length
...
var1<-data2$var1 #Guardando objetos en ambiente global
